...
async function createBookingsForPaidCheckout(supabase, checkoutData) {
  ...
  for (const item of items) {
    try {
      ...
      console.error("Pesapal IPN booking create error", error);
    }
  }
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Find checkout by merchant reference if possible, else by stored tracking ID
    ...
    if (!statusRes.ok) {
      console.error("Pesapal IPN status error:", statusData);
    ...
    if (!checkoutData) {
      // Acknowledge IPN anyway; you can reconcile later.
      return json(res, 200, {
        success: true,
        acknowledged: true,
    ...
    console.error("Pesapal IPN error", error?.providerResponse || error);
    return json(res, 500, { error: "IPN processing failed", message: error?.message || "Unexpected error" });
  }
}
