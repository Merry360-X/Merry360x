...
export default function HostDashboard() {
  ...
  const createProperty = async () => {
    ...
    if (isMonthlyOnly && monthlyPrice <= 0) {
      toast({
        ...
        description: "Set a monthly price for monthly-only listings.",
      });
      return null;
    }
  ...
  const getResolvedBookingAmountForHost = useCallback((booking: Booking) => {
    ...
    const listingSubtotal = Number(listing.listingSubtotal || 0);

    if (Number.isFinite(listingSubtotal) && listingSubtotal > 0) {
      const bookingCurrency = String(bookingAmount.currency || 'RWF').toUpperCase();
      const listingCurrency = String(listing.currency || 'RWF').toUpperCase();

      let comparableBookingAmount = Number(bookingAmount.amount || 0);
      if (bookingCurrency && listingCurrency && bookingCurrency !== listingCurrency) {
        const converted = convertAmount(comparableBookingAmount, bookingCurrency, listingCurrency, usdRates);
        if (Number.isFinite(Number(converted))) {
          comparableBookingAmount = Number(converted);
        }
      }

      if (!isCheckoutAmountReasonable(comparableBookingAmount, listingSubtotal)) {
        return {
          amount: listingSubtotal,
  ...
  const openRoomWizard = (hotel?: Property | null) => {
    ...
    setPropertyForm((f) => ({
      ...
      currency: hotel?.currency || f.currency,
      title: hotel?.title ? `${hotel.title} - Room` : "",
    }));
  ...
  const hasValidPropertyPricing = () => {
    if (propertyForm.listing_mode === "monthly_only") {
      return Number(propertyForm.price_per_month || 0) > 0;
    }
    const nightlyPrice = Number(propertyForm.price_per_night || 0);
    const monthlyPrice = Number(propertyForm.price_per_month || 0);
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <div>
                        <Label className="text-[10px]">
                          {tourPricingModel === "per_group"
                            ? "Price/Group"
                            : tourPricingModel === "per_hour"
                              ? "Price/Hour"
                              : tourPricingModel === "per_minute"
                                ? "Price/Minute"
                                : "Price/Person"}
                        </Label>
                        <Input
                          type="number"
                          value={form.price_per_person || 0}
                          onChange={(e) => setForm({ ...form, price_per_person: parseFloat(e.target.value) || 0 })}
                          min="0"
                          step="0.01"
                          className="mt-1 h-8 text-sm"
                        />
  ...
  if (showRoomWizard) {
    ...
    const hostHotels = (properties || []).filter(
      (property) =>
        String(property.property_type || "").toLowerCase() === "hotel" &&
              ...
              onSubmit={async (e) => {
                ...
                if (
                  ...
                  toast({
                    ...
                    description: "Choose the hotel this room belongs to so guests can view all rooms under that hotel.",
                  });
                  return;
                }
          ...
          <Card className="p-6 md:p-8">
            ...
            {wizardStep === 1 && (
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <div>
                    <Label className="text-base font-medium">Listing Mode</Label>
              ...
              <div className="space-y-6">
                <div className="text-center mb-8">
                  ...
                  <p className="text-muted-foreground mt-2">
                    {propertyForm.listing_mode === "monthly_only"
                      ? "Set your monthly price for long-stay guests."
                    ...
                    <div className="flex items-start justify-between">
                      ...
                      <div className="text-right">
                        <div className="text-2xl font-bold text-primary">
                          {propertyForm.listing_mode === "monthly_only"
                            ? formatMoney(Number(propertyForm.price_per_month || 0), propertyForm.currency)
                            : formatMoney(propertyForm.price_per_night, propertyForm.currency)}
                        </div>
                        <div className="text-sm text-muted-foreground">
                          {propertyForm.listing_mode === "monthly_only" ? "per month" : "per night"}
                        </div>
                        {propertyForm.weekly_discount > 0 && (
