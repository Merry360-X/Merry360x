...
const fetchTours = async ({
  ...
  if (packageList.length > 0 && !packagesRes.error) {
    const packagesAsTours: TourRow[] = packageList.map(pkg => ({
      ...
      rating: null,
      review_count: null,
      location: [pkg.city, pkg.country].filter(Boolean).join(", "),
...
const Tours = () => {
  ...
  useEffect(() => {
    ...
    setCountry(searchParams.get("country") ?? ALL_COUNTRIES_VALUE);
  ...
  const { data: tours = [], isError, isLoading: toursLoading, refetch: refetchTours } = useQuery({
    queryKey: [
      "tours",
      searchParams.get("q") ?? "",
      searchParams.get("country") ?? ALL_COUNTRIES_VALUE,
    ...
    queryFn: () =>
      fetchTours({
        ...
        country: searchParams.get("country") ?? ALL_COUNTRIES_VALUE,
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 lg:px-8 mb-12">
        <div className="bg-card rounded-2xl shadow-card p-3 sm:p-4 flex flex-col md:flex-row items-stretch md:items-center gap-3 md:gap-4 max-w-4xl mx-auto border border-border/50">
          <div className="flex-1 flex items-center gap-2 px-1 sm:px-3">
            <div className="w-full">
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  type="text"
                  placeholder="Ask Merry AI about tours and experiences"
                  value={query}
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 w-full md:w-auto">
            ...
            <Button variant="search" className="gap-2 w-full sm:w-auto shrink-0 shadow-sm" type="button" onClick={runSearch}>
