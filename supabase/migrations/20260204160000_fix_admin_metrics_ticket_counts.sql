-- Fix admin_dashboard_metrics function to actually count support tickets
-- The support_tickets table now exists, so we can query it properly
...
BEGIN
    SELECT jsonb_build_object(
        ...
        'vehicles_live', (
            SELECT COALESCE(COUNT(*), 0)::int FROM transport_vehicles WHERE is_available = true
        ),
        ...
        -- Support ticket metrics - NOW ACTUALLY COUNTING!
        'tickets_total', (
        ...
        'incidents_open', 0,
        'blacklist_count', (
            SELECT COALESCE(COUNT(*), 0)::int FROM user_roles WHERE role = 'guest'
        ),
...
COMMENT ON FUNCTION admin_dashboard_metrics() IS 'Returns comprehensive dashboard metrics for admin users including support ticket counts';
