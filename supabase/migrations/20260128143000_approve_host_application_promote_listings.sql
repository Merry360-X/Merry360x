-- Approve host application safely: grant host role and promote application-created listings

ALTER TABLE public.host_applications
  ADD COLUMN IF NOT EXISTS promoted_property_id uuid,
  ADD COLUMN IF NOT EXISTS promoted_tour_id uuid,
  ADD COLUMN IF NOT EXISTS promoted_vehicle_id uuid;

COMMENT ON COLUMN public.host_applications.promoted_property_id IS 'Property created from this host application (if any)';
COMMENT ON COLUMN public.host_applications.promoted_tour_id IS 'Tour created from this host application (if any)';
COMMENT ON COLUMN public.host_applications.promoted_vehicle_id IS 'Transport vehicle created from this host application (if any)';
