-- Fix admin_dashboard_metrics function to remove references to non-existent columns
-- The issue: Function tries to access profiles.is_suspended and profiles.is_verified which don't exist
...
DECLARE
    result JSONB;
BEGIN
    -- Build metrics without referencing non-existent columns
    SELECT jsonb_build_object(
        ...
        'users_suspended', 0, -- No suspension column exists, default to 0
        'hosts_total', (
        ...
        -- Content metrics
        'stories_total', 0, -- Stories functionality not implemented yet
        'properties_total', (
        ...
        'bookings_paid', (
            SELECT COALESCE(COUNT(*), 0)::int 
            FROM bookings 
            WHERE status = 'completed'  -- Completed bookings are considered paid
        ),
        ...
        'revenue_gross', (
            ...
            FROM bookings 
            WHERE status IN ('confirmed', 'completed')  -- Only confirmed and completed bookings generate revenue
        ...
        'revenue_by_currency', (
            ...
            FROM (
                ...
                FROM bookings 
                WHERE status IN ('confirmed', 'completed')  -- Only confirmed and completed
                AND total_price IS NOT NULL
                GROUP BY currency
        ...
        'reviews_hidden', 0, -- No hidden flag exists
        
        -- Order metrics  
        'orders_total', (
            SELECT COALESCE(COUNT(*), 0)::int FROM trip_cart_items
        ),
        'refunds_total', 0, -- No refunds table exists
        
        -- Support metrics (defaulting to 0 since tables don't exist)
        'tickets_total', 0,
        'tickets_open', 0,
...
-- Add comment
COMMENT ON FUNCTION admin_dashboard_metrics() IS 'Returns comprehensive dashboard metrics for admin users. Fixed to remove references to non-existent columns.';
